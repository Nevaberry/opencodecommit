...
Run targeted OpenCodeCommit e2e suites with backend filtering and a plain-text log.

Options:
  -b, --backends CSV|all   Backends to enable, for example codex or codex,claude
  -t, --target TARGET      extension, tui, or both
  -l, --log-file PATH      Log file path. Defaults to .logs/e2e-<timestamp>.log
  -h, --help               Show this help

Examples:
  scripts/test-e2e.sh --backends codex --target both
