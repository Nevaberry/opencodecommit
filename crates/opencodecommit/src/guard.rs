...
use opencodecommit::sensitive::scan_diff_for_sensitive_content;
...
fn run_managed_hook_with_state(
    ...
    if hook_name == "pre-commit" && !allow_sensitive_bypass() {
        ...
        if !diff.is_empty() {
            ...
            let report = scan_diff_for_sensitive_content(&diff, &changed_files);
...
fn allow_sensitive_bypass() -> bool {
    matches!(
...
mod tests {
    ...
    fn pre_commit_blocks_sensitive_diff_and_bypass_preserves_other_hooks() {
        ...
        with_repo(&repo, || {
            ...
            let blocked =
                run_managed_hook_with_state(&paths.hooks_dir, &metadata, "pre-commit", &[])
                    .unwrap();
            assert_eq!(blocked, 1);
